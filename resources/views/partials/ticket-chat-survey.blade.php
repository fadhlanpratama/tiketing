    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 shadow-sm">
        ...
        <div class="space-y-4 max-h-[420px] overflow-y-auto pr-1">
            @forelse($ticket->messages as $msg)
                @php
                    ...
                    $pesanPanjang = $msg->pesan && (mb_strlen($msg->pesan) > 150 || substr_count($msg->pesan, "\n") > 2);
                @endphp
                ...
                <div class="flex gap-3 p-3.5 rounded-xl border {{ $isMsgPj ? 'bg-slate-50/70 border-slate-200' : 'bg-white border-slate-100 shadow-sm' }}">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        {{-- Teks Pesan (dibatasi 3 baris) --}}
                        @if($msg->pesan)
                        <p class="js-pesan-text text-xs sm:text-sm text-slate-600 whitespace-pre-line leading-relaxed break-words [overflow-wrap:anywhere] mt-1 line-clamp-3">
                            {{ $msg->pesan }}
                        </p>
                        @if($pesanPanjang)
                        <button type="button" onclick="toggleFullText(this)" class="js-toggle-text text-[11px] font-bold text-blue-600 hover:text-blue-700 mt-1 cursor-pointer">
                            Selengkapnya
                        </button>
                        @endif
                        @endif

                        {{-- Lampiran Foto (thumbnail kecil) --}}
                        @if($msg->foto)
                        <div class="mt-2">
                            <a href="{{ asset('storage/' . $msg->foto) }}" target="_blank" rel="noopener" class="inline-block">
                                <img src="{{ asset('storage/' . $msg->foto) }}" alt="Lampiran foto"
                                     loading="lazy"
                                     class="w-20 h-20 object-cover rounded-lg border border-slate-200 hover:opacity-90 transition cursor-zoom-in">
                            </a>
                        </div>
                        @endif
                    </div>
        ...
        <form action="{{ route($chatRoute, $ticket->id) }}" method="POST" enctype="multipart/form-data" class="mt-4 pt-4 border-t border-slate-100">
            ...
            <div class="space-y-2">
                ...
                <textarea name="pesan" id="pesan" rows="3" maxlength="1000"
                    ...
                    placeholder="Tulis balasan atau instruksi tambahan di sini...">{{ old('pesan') }}</textarea>
                @error('pesan')
                ...
                {{-- Input Foto + Preview --}}
                <div>
                    <label for="foto" class="block text-xs font-semibold text-slate-700 mb-1">Lampirkan Foto (opsional)</label>
                    <div class="flex items-center gap-3">
                        <label for="foto" class="cursor-pointer bg-slate-100 hover:bg-slate-200 text-slate-600 font-semibold text-xs px-3.5 py-2.5 rounded-xl transition flex items-center gap-2 shrink-0">
                            <i class="fa-solid fa-camera text-xs"></i>
                            <span>Pilih Foto</span>
                        </label>
                        <input type="file" name="foto" id="foto" accept="image/jpeg,image/png,image/jpg" class="hidden" onchange="previewFotoKomentar(this)">
                        <span id="fotoFileName" class="text-xs text-slate-400 italic truncate">Belum ada foto dipilih</span>
                    </div>

                    <div id="fotoPreviewWrap" class="hidden mt-2 relative inline-block">
                        <img id="fotoPreviewImg" src="" alt="Preview" class="w-20 h-20 rounded-lg border border-slate-200 object-cover">
                        <button type="button" onclick="clearFotoKomentar()" class="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px] shadow-md cursor-pointer">
