...
<head>
    ...
    <title>ESDM - Tiketing - Edit Pengguna</title>
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-sm flex items-center justify-between gap-4">
            <div class="space-y-1.5">
                <h2 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Edit Pengguna</h2>
                <p class="text-xs sm:text-sm text-slate-500">Perbarui informasi profil, role, status, atau kata sandi akun <span class="font-bold text-slate-700">{{ $user->nama_lengkap }}</span>.</p>
            </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 sm:p-8">
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-8" id="editUserForm" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            @php $selectedDivisi = old('divisi', $user->divisi); @endphp
                            <input type="hidden" name="divisi" id="divisi" value="{{ $selectedDivisi }}">
                            <div class="relative w-full custom-dropdown" id="divisiDropdown">
                                <button type="button" class="dropdown-btn w-full bg-slate-50 border {{ $errors->has('divisi') ? 'border-red-400 bg-red-50/10' : 'border-slate-200' }} py-3 pl-10 pr-4 rounded-xl text-xs font-semibold text-slate-800 flex justify-between items-center cursor-pointer hover:bg-slate-100/80 transition focus:ring-2 focus:ring-amber-400 outline-none">
                                    ...
                                    <span class="dropdown-label truncate {{ $selectedDivisi ? 'text-slate-800' : 'text-slate-400' }}">
                                ...
                                <div class="dropdown-menu hidden absolute left-0 right-0 z-50 mt-1.5 max-h-56 overflow-y-auto bg-white border border-slate-200 shadow-2xl rounded-2xl p-1.5 space-y-0.5 text-xs text-slate-700 font-medium">
                                    @foreach($daftarDivisi as $d)
                                        <div data-value="{{ $d }}" class="dropdown-item px-3 py-2.5 rounded-xl cursor-pointer hover:bg-slate-50 transition flex items-center justify-between {{ $selectedDivisi == $d ? 'bg-amber-50 text-amber-900 font-bold' : '' }}">
                                            <span>{{ $d }}</span>
                                            @if($selectedDivisi == $d) <i class="fa-solid fa-check text-amber-600 text-xs"></i> @endif
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="relative w-full custom-dropdown" id="roleDropdown">
                                <button type="button" class="dropdown-btn w-full bg-slate-50 border {{ $errors->has('role') ? 'border-red-400 bg-red-50/10' : 'border-slate-200' }} py-3 pl-10 pr-4 rounded-xl text-xs font-semibold text-slate-800 flex justify-between items-center cursor-pointer hover:bg-slate-100/80 transition focus:ring-2 focus:ring-amber-400 outline-none">
                                    ...
                                    <span class="dropdown-label truncate text-slate-800">
                                        @if($selectedRole == 'user') User (Pengguna Biasa)
                                        @elseif($selectedRole == 'pj') PJ Teknisi (Penanggung Jawab)
                                        @elseif($selectedRole == 'admin') Admin (Pengelola Sistem)
                                        @else Pilih Role
                                        @endif
                                    </span>
                                    <i class="fa-solid fa-chevron-down text-[10px] text-slate-400 transition-transform duration-200"></i>
                                ...
                                <div class="dropdown-menu hidden absolute left-0 right-0 z-50 mt-1.5 bg-white border border-slate-200 shadow-2xl rounded-2xl p-1.5 space-y-0.5 text-xs text-slate-700 font-medium">
                                    <div data-value="user" class="dropdown-item px-3 py-2.5 rounded-xl cursor-pointer hover:bg-slate-50 transition flex items-center justify-between {{ $selectedRole == 'user' ? 'bg-amber-50 text-amber-900 font-bold' : '' }}">
                                        <span>User (Pengguna Biasa)</span>
                                        @if($selectedRole == 'user') <i class="fa-solid fa-check text-amber-600 text-xs"></i> @endif
                                    </div>
                                    <div data-value="pj" class="dropdown-item px-3 py-2.5 rounded-xl cursor-pointer hover:bg-slate-50 transition flex items-center justify-between {{ $selectedRole == 'pj' ? 'bg-amber-50 text-amber-900 font-bold' : '' }}">
                                        <span>PJ Teknisi (Penanggung Jawab)</span>
                                        @if($selectedRole == 'pj') <i class="fa-solid fa-check text-amber-600 text-xs"></i> @endif
                                    </div>
                                    <div data-value="admin" class="dropdown-item px-3 py-2.5 rounded-xl cursor-pointer hover:bg-slate-50 transition flex items-center justify-between {{ $selectedRole == 'admin' ? 'bg-amber-50 text-amber-900 font-bold' : '' }}">
                                        <span>Admin (Pengelola Sistem)</span>
                                        @if($selectedRole == 'admin') <i class="fa-solid fa-check text-amber-600 text-xs"></i> @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            @php $selectedStatus = old('status', $user->status); @endphp
                            <input type="hidden" name="status" id="status" value="{{ $selectedStatus }}">
                            <div class="relative w-full custom-dropdown" id="statusDropdown">
                                <button type="button" class="dropdown-btn w-full bg-slate-50 border {{ $errors->has('status') ? 'border-red-400 bg-red-50/10' : 'border-slate-200' }} py-3 pl-10 pr-4 rounded-xl text-xs font-semibold text-slate-800 flex justify-between items-center cursor-pointer hover:bg-slate-100/80 transition focus:ring-2 focus:ring-amber-400 outline-none">
                                    <i class="fa-solid fa-signal absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                                    <span class="dropdown-label truncate text-slate-800">
                                        @if($selectedStatus == 'active') Aktif
                                        @elseif($selectedStatus == 'rejected') Ditolak / Nonaktif
                                        @else Pilih Status
                                        @endif
                                    </span>
                                    <i class="fa-solid fa-chevron-down text-[10px] text-slate-400 transition-transform duration-200"></i>
                                ...
                                <div class="dropdown-menu hidden absolute left-0 right-0 z-50 mt-1.5 bg-white border border-slate-200 shadow-2xl rounded-2xl p-1.5 space-y-0.5 text-xs text-slate-700 font-medium">
                                    <div data-value="active" class="dropdown-item px-3 py-2.5 rounded-xl cursor-pointer hover:bg-slate-50 transition flex items-center justify-between {{ $selectedStatus == 'active' ? 'bg-amber-50 text-amber-900 font-bold' : '' }}">
                                        <span>Aktif</span>
                                        @if($selectedStatus == 'active') <i class="fa-solid fa-check text-amber-600 text-xs"></i> @endif
                                    </div>
                                    <div data-value="rejected" class="dropdown-item px-3 py-2.5 rounded-xl cursor-pointer hover:bg-slate-50 transition flex items-center justify-between {{ $selectedStatus == 'rejected' ? 'bg-amber-50 text-amber-900 font-bold' : '' }}">
                                        <span>Ditolak / Nonaktif</span>
                                        @if($selectedStatus == 'rejected') <i class="fa-solid fa-check text-amber-600 text-xs"></i> @endif
                                    </div>
                                </div>
                            </div>
                            <span class="error-msg text-xs text-red-500 font-medium mt-1.5 pl-0.5 flex items-center gap-1.5 {{ $errors->has('status') ? '' : 'hidden' }}" id="err-status">
                                <i class="fa-solid fa-circle-exclamation text-xs"></i> <span>{{ $errors->first('status') }}</span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6 pt-4 border-t border-slate-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        {{-- Konfirmasi Password --}}
                        <div>
                            <label for="password_confirmation" class="block text-xs font-bold text-slate-700 mb-2">Konfirmasi Kata Sandi Baru</label>
                            <div class="relative flex items-center">
